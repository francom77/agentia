...
//parametros del transporte
	//el formato de los parametros es destino tipo categoria precio Decuento1 descuentoN --- Agencia1 AgenciaN ---
	//donde descuento es un string del tipo cant-min-max y agencia un string con el aid.

	private String destino, tipo;
	...
	protected void setup(){
		System.out.println("Agente Transporte "+getAID().getName()+" fue creado.");
		...
		for (String elem: this.agencias){
			...
			String tipo = "transporte-"+this.destino+"-"+elem;
			sd.setType(tipo);
			sd.setName("JADE-transporte-"+elem);
			dfd.addServices(sd);
		}
	...
	protected void takeDown() {
		...
		System.out.println("Agente Transporte "+getAID().getName()+" finalizado.");
	}
		...
		public void action() {
			...
			if (msg != null) {
				...
				if (tipoCond && categoriaCond) {
					//El transporte cumple con el tipo (colectivo, avión, etc) y la categoria deseada, proponer precio.
					reply.setPerformative(ACLMessage.PROPOSE);
					reply.setContent(String.valueOf(precio) +"@"+tipo+"@"+categoria);
				}
				else {
					// El transporte no cumple con la categoria o tipo de transporte.
					reply.setPerformative(ACLMessage.REFUSE);
					reply.setContent("transporte-no-adecuado");
				}
				myAgent.send(reply);
			}
			else {
	...
	private class ServidorDeRenegociaciones extends CyclicBehaviour {
		public void action() {
			...
			if (msg != null) {
				...
				Integer cantidadPersonas = Integer.parseInt(requerimientoArray[1]);
				ACLMessage reply = msg.createReply();
				...
				for (Descuento elem : descuentos){
					if (cantidadPersonas >= elem.getCantidad()){
